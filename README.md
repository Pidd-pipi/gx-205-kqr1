...
- 智能组卷练习：支持按难度和题量生成练习卷，规则如下：
  - **带编号试卷快照**：每次生成都会持久化一份带编号（如 `JP-0001`）的快照，同一编号再次打开时题目、顺序与难度标记完全不变；也可通过历史列表回查。
  - **卷内不重题**：题池题目在一套卷内最多出现一次，不会循环填充凑数。
  - **题型均衡**：五种题型数量相差不超过 1。
  - **相邻难度补入**：目标难度题目不足时，从难度轴上距离最近的相邻难度逐层补入，并在替换说明中写明从哪些难度各补入多少题，题目保留原始难度标记。
  - **真实缺口**：题池仍不足时只返回实际可出题量，并按题型列出缺口（练习页以红色标签提示），绝不重复填充。
...
### 组卷规则本地验证（无需 PostgreSQL）

后端提供两个基于内存 SQLite 的自检脚本，验证不重题、题型均衡、相邻难度补入、缺口上报与快照回查：

```bash
cd backend
python3 scripts/verify_paper_rules.py   # 组卷规则单元级验证
python3 scripts/verify_paper_api.py     # HTTP API 端到端验证
```
